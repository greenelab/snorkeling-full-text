...
def clear_citations(open_token, close_token, section_text):
    ...
    while parenthesis_index_open != -1 and parenthesis_index_close != -1:
        parenthesis_string = section_text[parenthesis_index_open:parenthesis_index_close]
        cleaned_string = re.sub(r"\.", "", parenthesis_string)
        section_text = section_text.replace(parenthesis_string, cleaned_string)
        parenthesis_index_open = section_text.find(open_token, parenthesis_index_open+1)
        parenthesis_index_close = section_text.find(close_token, parenthesis_index_close+1)
...
def get_sql_result(sql_query, conn):
    ...
    return [
        ...
        for result_row in result_proxy
    ]

def insert_entities(
    file_open, output_file, 
    already_seen = set(), pmcid_map=None,
    entity_id_start = 1, skip_documents=False,
    ...
    with lzma.open(file_open, "rt") as infile, open(output_file, file_mode) as outfile:
        ...
        for line in tqdm.tqdm(reader):
            ...
            if (pubmed_id, int(line['offset']), int(line['end'])) in seen_entity[pubmed_id]:
                continue
            ...
            seen_entity[pubmed_id].add((pubmed_id, int(line['offset']), int(line['end'])))
                ...
                with open(f"output/files/{document_id}.tsv", "w") as outfile:
                    ...
                    for passage in document.xpath(path_map['passage_path']):
                        ...
                        if section not in [
                            ...
                            section = (
                                "INTRODUCTION" if section == "INTRO" else
                                "CONCLUSION" if section == "CONCL" else
                                "SUPPLEMENTAL" if section == "SUPPL" else
                                section
                            )
                        ...
                        for position, sentence in enumerate(doc_sentences):
                            (
                                ...
                                zip(*[
                                    (
                                        repr(tok.text), repr(tok.tag_), 
                                        repr(tok.lemma_.lower()), repr(tok.dep_),
                                        str(tok.idx+offset)
                                    )
                                    for tok in sentence
                                ])
                            ...
                            writer.writerow({
                                    ...
                                    "position": position + section_counter[section], # sections are broken by paragraphs and titles
                                    "text": sentence.text,
                                    "word": "|".join(word),
                                    "pos_tag":"|".join(pos_tag),
                                    "lemma": "|".join(lemma),
                                    "dep":"|".join(dep),
                                    "char_offset": "|".join(char_offset), # act like title and abstract are one string
                            })
